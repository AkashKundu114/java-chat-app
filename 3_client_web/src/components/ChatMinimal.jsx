...
const ChatMinimal = () => {
  const { messages, contacts, user, authStatus, login, sendMessage } = useChatBridge();
        ...
        <div className="z-10 w-full max-w-sm p-8 bg-white border border-gray-200 shadow-xl rounded-2xl">
          <div className="mb-8 text-center">
            ...
            <h2 className="text-2xl font-extrabold text-gray-800">
              {authStatus === 'disconnected' ? 'Connecting...' : isRegisterMode ? 'Create Account' : 'Welcome Back'}
            </h2>
          ...
          <form onSubmit={handleAuthSubmit} className="space-y-5">
            <div>
              ...
              <input 
                className="w-full border border-gray-200 rounded-lg px-4 py-2.5 focus:border-yellow-400 focus:ring-4 focus:ring-yellow-50 outline-none transition-all bg-gray-50 font-medium"
                type="text" 
                value={loginUser}
                onChange={e => setLoginUser(e.target.value)}
                disabled={authStatus === 'disconnected'}
            ...
            <button 
              type="submit"
              disabled={authStatus === 'disconnected'}
              className="w-full py-3 text-sm font-bold text-white transition-all duration-200 transform bg-gray-900 rounded-lg shadow-md hover:bg-black hover:shadow-lg active:scale-95"
            >
          ...
          <p className="mt-6 text-xs font-medium text-center text-gray-500">
            ...
            <span className="ml-1 text-yellow-600 cursor-pointer hover:underline" onClick={() => setIsRegisterMode(!isRegisterMode)}>
  ...
  return (
    <div className="flex h-screen overflow-hidden bg-white">
      ...
      <div className={`
        ...
        md:flex w-full md:w-80 flex-col bg-gray-100 border-r border-gray-200 z-20 transition-all duration-300
      `}>
        <div className="p-6">
          <div className="mb-6">
            ...
            <p className="text-xs font-medium text-gray-500 mt-1.5">
              Logged in as <span className="text-gray-900">{user}</span>
            </p>
          </div>
            ...
            >
              <div className="flex items-center gap-3">
                <div className={`relative w-10 h-10 rounded-full flex items-center justify-center font-bold text-sm transition-colors ${
                   activeContact?.id === contact.id ? 'bg-gray-700 text-white' : 'bg-white text-gray-600 group-hover:bg-white group-hover:shadow-sm'
                }`}>
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex justify-between items-baseline mb-0.5">
                    <span className={`text-sm font-bold truncate ${
                        activeContact?.id === contact.id ? 'text-white' : 'text-gray-800'
                    }`}>
                        {contact.name}
                    </span>
                  </div>
      ...
      <div className={`
        ...
        md:flex relative flex-col flex-1 min-w-0 bg-white
      `}>
          ...
          {activeContact ? (
            <div className="flex items-center gap-3">
              ...
              <button 
                ...
                className="p-1 mr-1 text-gray-600 rounded-full md:hidden hover:bg-gray-200"
              >
          ...
          {allMessages
            .filter(msg => 
               activeContact && (
                 ...
                 (msg.realSenderName === user && msg.to === activeContact.name)
               )
            ) 
            .map((msg, index) => {
            ...
            return (
              <div key={index} className={`flex ${isMe ? 'justify-end' : 'justify-start'} animate-fade-in-up`}>
                <div 
                  className={`max-w-[85%] px-5 py-3 rounded-2xl shadow-sm text-sm relative border ${
                    isMe 
                    ? 'bg-yellow-100 border-yellow-200 text-gray-800 rounded-br-none' 
                    : 'bg-[#E1F5FE] border-[#E1F5FE] text-gray-900 rounded-bl-none'   
                  }`}
                >
                  <p className="leading-relaxed whitespace-pre-wrap">{msg.text}</p>
                  <span className={`text-[10px] block mt-1.5 font-bold ${isMe ? 'text-yellow-700/60' : 'text-blue-900/40'} text-right`}>
                    {msg.time}
                  </span>
                </div>
              </div>
        ...
        <div className="z-20 p-3 md:p-4">
          <div className="relative flex items-center max-w-4xl gap-2 p-2 mx-auto bg-white border border-gray-100 shadow-lg rounded-2xl">
            ...
            {showEmojiPicker && (
              <div 
                ref={emojiPickerRef}
                className="absolute right-0 grid w-64 h-64 grid-cols-6 gap-2 p-3 overflow-y-auto bg-white border border-gray-200 shadow-xl bottom-16 rounded-2xl animate-fade-in-up"
                style={{ zIndex: 100 }}
              >
                {POPULAR_EMOJIS.map((emoji, idx) => (
                  <button
                    ...
                    className="p-1 text-xl transition-colors rounded-md hover:bg-gray-100"
                  >
                    {emoji}
                  </button>
                ))}
            ...
            <form onSubmit={handleSendMessage} className="flex-1">
              <input
                ...
                placeholder={activeContact ? "Write a message..." : ""}
                disabled={!activeContact}
                className="w-full px-2 py-2 text-sm font-medium text-gray-800 placeholder-gray-400 bg-transparent border-none focus:ring-0"
              />
            </form>
            ...
            <button
              ...
              className={`p-2.5 rounded-xl transition-all ${
                showEmojiPicker ? 'text-yellow-500 bg-yellow-50' : 'text-gray-400 hover:text-yellow-500 hover:bg-yellow-50'
              }`}
            >
              <svg xmlns="http://www.w3.org/2000/svg" className="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            <button 
              ...
              disabled={!inputValue.trim() || !activeContact}
              className="bg-gray-900 hover:bg-black disabled:bg-gray-200 disabled:text-gray-400 text-white rounded-xl p-2.5 transition-all shadow-md transform active:scale-95"
            >
              <svg xmlns="http://www.w3.org/2000/svg" className="w-5 h-5 transform rotate-90 translate-x-px" viewBox="0 0 20 20" fill="currentColor">
